<?php
    ...
    $provider_id = isset($_POST["ProviderID"]) ? $_POST["ProviderID"] : '';

    $connect = mysqli_connect("localhost", "root", "", "proteksyon");
    $output = '';

    $query = "
    SELECT provider_id, provider_email, provider_contactno, provider_name, provider_address, provider_longitude, provider_latitude
    ...
    $result = mysqli_query($connect, $query);
        ...
        if(mysqli_num_rows($result) > 0){
            ...
            {
                    $output .= '
                    <h4>Modifiying '. $row['provider_name'] .'</h4>
                    <div class="col">
                    <input type="text" style="visibility:hidden;" class="form-control" id="provider_id" name="provider_id" required value="'. $row['provider_id'] .'">
                    </div>
                    <div class="col">
