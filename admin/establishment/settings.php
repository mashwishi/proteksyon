        ...
        $(document).ready(function(){ 
            ...
            function getSettings(query)
            {
                ...
                success:function(data)
                {
                    ...
                    checkUpdate();
                    checkPassword();
                    $('#xx_establishment_id').val(EstablishmentID);
                }
                });
            ...
            function checkUpdate()
            {
                ...
                if(msgUpdateInfo == 0){
                    ...
                    //Error not checked
                    $("#alertPassword").html('<div class="alert alert-danger alert-dismissible fade show" role="alert"><strong>Error!</strong> Failed to update your password, Password confirmation error. <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                }
                else{
                    //none
                }
            ...
            function checkPassword()
            {
                const queryString = window.location.search;
                const urlParams = new URLSearchParams(queryString);
                const msgUpdatePass = urlParams.get('updatePassword');

                if(msgUpdatePass == 0){
                    //Success
                    $("#alertPassword").html('<div class="alert alert-success alert-dismissible fade show" role="alert"><strong>Success!</strong> Your information has been successfully updated.<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                }
                else if(msgUpdatePass == 1){
                    //Error not checked
                    $("#alertPassword").html('<div class="alert alert-danger alert-dismissible fade show" role="alert"><strong>Error!</strong> Failed to update your settings, You must agree to change your information to update your information.<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                }
                else if(msgUpdatePass == 2){
                    //Error not checked
                    $("#alertPassword").html('<div class="alert alert-warning alert-dismissible fade show" role="alert"><strong>Error!</strong> Failed to update your settings, You must not leave any blanks to change your information to update your information.<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                }
                else if(msgUpdatePass == 3){
                    //Error not checked
                    $("#alertPassword").html('<div class="alert alert-danger alert-dismissible fade show" role="alert"><strong>Error!</strong> Failed to update your settings, There is a problem to database connection! Please try again.<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                }
                else if(msgUpdatePass == 4){
                    //Error not checked
                    $("#alertPassword").html('<div class="alert alert-danger alert-dismissible fade show" role="alert"><strong>Error!</strong> Failed to update your password, Password confirmation error. <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                }
                else{
		...
		<main>
			<div class="head-title">
				<div class="left">
					...
					<ul class="breadcrumb">
						...
						<li>
							<a class="active" href="/admin/establishment">Establishment</a>
						</li>
					</ul>
				</div>
			</div>
			...
			<div class="table-data">
				<div class="order">
                    ...
                    <div class="container">
                        <div class="row">
                            <form id="passSettings"  action="./data/changePassword.php" method="post" enctype="multipart/form-data">
                            <input type="text" style="visibility:hidden;" class="form-control" id="xx_establishment_id" name="xx_establishment_id" required>
                                <h4>Password</h4>
                                <div class="col">
                                    <div class="row row-cols-2" style="margin-bottom: 10px;">
                                        <div class="col">
                                            <label for="inputNewPass">New Password</label>
                                            <input type="password" class="form-control" id="inputNewPass" name="newPassowrd">
                                        </div>
                                        <div class="col">
                                            <label for="inputRePass">Confirm Password</label>
                                            <input type="password" class="form-control" id="inputRePass"  name="conPassowrd">
                                        </div>
                                    </div>
                                    <div id="alertPassword"></div>
                                    <div class="form-group" style="margin-bottom: 10px;">
                                        <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="xgridCheck" value="0" name="cAgreeUpdate">
                                        <label class="form-check-label" for="xgridCheck">
