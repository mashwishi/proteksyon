  ...
  <head>
    ...
    <body>
            ...
            if($UUID != '' || $UUID != null)
            {
                ...
                $stmt = $conn->prepare("SELECT * FROM establishment_tb WHERE establishment_uuid=?");
				$stmt->execute([$UUID]);

                if ($stmt->rowCount() === 1) {

                    $user = $stmt->fetch();
                    $user_verification = $user['verified'];	

                        if($user_verification == 0){

                            $sql = "UPDATE establishment_tb SET verified=1 WHERE establishment_uuid = '$UUID'";               
