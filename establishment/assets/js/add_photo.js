...
function frontreadURL(finput) {
  if (finput.files && finput.files[0]) {
    ...
    freader.onload = function(e) {
      ...
      var business_filetype = extension(finput.files[0].name).toLowerCase();

      if( business_filetype == "pdf" || business_filetype == "doc" || business_filetype == "docx"){
        var file_nameandtype =  finput.files[0].name;
        const file_nameandtypes = file_nameandtype.substring(0, 10).concat('...');
        $('.file-title').html('<h1><strong>Document Attached:</strong><br/>' + file_nameandtypes + '</h1>');
